...
Word wrap hides the fixed-height line-number gutter to avoid displaying misleading line positions. Browser-created documents are temporary. Browser preview has no shell; the terminal requires the desktop application. This change does not add workspace-wide content search, multiple cursors, or Save As.
