...
/**
 * The orders the "Sort Changes" menu offers -- exactly these three, nothing more:
 * - `discovery`: the order Git reported the files in (untouched);
 * - `name`: by file name, then by folder;
 * - `status`: grouped by status letter (see `STATUS_ORDER`), then by path.
 */
export type ChangesSort = "discovery" | "name" | "status";

export const CHANGES_SORTS: readonly ChangesSort[] = ["discovery", "name", "status"];
 ...
 *
 * `name` compares the file name without regard to case or folder ("a.ts" in two folders
 * sit together, "file2" before "file10"), then the folder, then the full path. `status`
 * groups by `statusLetter`, then by path. A renamed file sorts by its new path. Ties fall
 * back to Git's own order (`Array.prototype.sort` is stable), so equal files never shuffle
 * between refreshes.
 */
export function sortChanges(entries: readonly GitEntry[], order: ChangesSort): GitEntry[] {
  ...
  if (order === "discovery") return [...conflicts, ...others];
...
/** The remembered choice, or `discovery` when nothing valid was saved. */
export function readChangesSort(): ChangesSort {
  try {
    const saved = localStorage.getItem(STORAGE_KEY);
    return CHANGES_SORTS.includes(saved as ChangesSort) ? (saved as ChangesSort) : "discovery";
  } catch {
    return "discovery";
  }
}
