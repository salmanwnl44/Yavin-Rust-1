...
export interface SearchHit {
  path: string; line: number; text: string; start: number; end: number;
}
export interface SearchResult { hits: SearchHit[]; warning: string; truncated: boolean }
...
export function parseSearch(output: ToolOutput, folder: string, bufferPath?: string): SearchResult {
  ...
  for (const record of output.stdout.split("\n")) {
    ...
    let event;
    try { event = JSON.parse(record); }
    catch { if (output.truncated) break; throw new Error("Invalid search output"); }
    if (event.type === "summary") completed = true;
    if (event.type === "end" && event.data.binary_offset != null) skipped++;
    if (event.type !== "match") continue;
    const data = event.data;
    if (typeof data.lines?.text !== "string" || (!bufferPath && typeof data.path?.text !== "string")) {
      skipped++; continue;
    }
    const text: string = data.lines.text;
    const path = bufferPath ?? `${folder.replace(/\/$/, "")}/${data.path.text.replace(/\\/g, "/").replace(/^\.\//, "")}`;
    for (const match of data.submatches) {
      hits.push({ path, line: data.line_number, text, start: byteToColumn(text, match.start), end: byteToColumn(text, match.end) });
    }
  }
  // Exit code 2 with a summary means some paths failed (e.g. locked files); without one, rg stopped early.
  if (output.code > 1 && !completed && !output.truncated) throw new Error(output.stderr || "Search failed");
  return { hits, truncated: output.truncated || output.code > 1, warning: [output.stderr.trim(), skipped ? `${skipped} binary or unsupported records skipped.` : ""].filter(Boolean).join(" ") };
}

export async function searchWorkspace(workspace: string, options: SearchOptions, buffers: Record<string, string>, openOnly: boolean, signal: AbortSignal): Promise<SearchResult> {
  const call = async (request: SearchOptions) => {
    if (signal.aborted) throw new Error("Cancelled");
    const id = crypto.randomUUID();
    const cancel = () => { void native("cancel_search", { id }).catch(() => {}); };
  ...
  if (paths.length) {
    ...
    if (listed.truncated || (listed.code > 1 && !listed.stdout)) throw new Error(listed.stderr || "File list is incomplete; narrow search scope");
    if (listed.code > 1) result.truncated = true;
    const allowed = new Set(listed.stdout.split("\0").filter(Boolean).map(p => `${options.folder.replace(/\/$/, "")}/${p.replace(/\\/g, "/").replace(/^\.\//, "")}`));
    result.hits = result.hits.filter(hit => buffers[hit.path] === undefined);
    for (const path of paths) {
      if (!allowed.has(path)) continue;
      const overlay = parseSearch(await call({ ...options, include: [], exclude: [], buffer: buffers[path] }), options.folder, path);
  ...
  if (result.hits.length > 10000) { result.hits.length = 10000; result.truncated = true; }
  return result;
}
